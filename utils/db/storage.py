...
class DatabaseManager(object):
    ...
    def create_tables(self):
        self.query('CREATE TABLE IF NOT EXISTS products (idx text, title text, body text, '
                   'photo blob, price int, tag text)')
        self.query('CREATE TABLE IF NOT EXISTS orders (cid int, usr_name text, usr_address text, '
                   'products text, extra text, phone text, title text)')
        self.query('CREATE TABLE IF NOT EXISTS cart (cid int, idx text, quantity int, titles text)')
...
orders: cid int, usr_name text, usr_address text, products text, title text
