...
async def process_cart(message: Message, state: FSMContext):
    ...
    if len(cart_data) == 0:
        ...
        for _, idx, count_in_cart, title_of_ordered in cart_data:
            ...
            if product is None:
                ...
                async with state.proxy() as data:
                    data['products'][idx] = [title, price, count_in_cart, title_of_ordered]
...
async def checkout(message, state):
    ...
    async with state.proxy() as data:
        print(data['products'])
        for title, price, count_in_cart, title_of_ordered in data['products'].values():
...
async def process_address(message: Message, state: FSMContext):
    ...
    await message.answer('Оставьте номер своего телефона для связи')
    await CheckoutState.next()
...
async def process_phone_number(message: Message, state: FSMContext):
    ...
    await message.answer('Добавьте ваше пожелание к заказу: ')
    await CheckoutState.next()


@dp.message_handler(IsUser(), text=back_message, state=CheckoutState.extra)
async def process_extra_wish_back(message: Message, state: FSMContext):

    await CheckoutState.phone_number.set()
    async with state.proxy() as data:
        await message.answer('Изменить номер с <b>' + data['phone'] + '</b>?',
                             reply_markup=back_markup())


@dp.message_handler(IsUser(), state=CheckoutState.extra)
async def process_extra_wish(message: Message, state: FSMContext):
...
async def process_confirm_back(message: Message):

    await CheckoutState.extra.set()

    await message.answer('Изменить ваше пожелание к заказу?', reply_markup=back_markup())
    ...
    if enough_money:
        ...
        async with state.proxy() as data:
            ...
            for i in data['products'].keys():
                title += f'{data["products"][i][0]};'
            db.query('INSERT INTO orders VALUES (?, ?, ?, ?, ?, ?, ?)',
                     (cid, data['name'], data['address'], ' '.join(products), data['extra'], data['phone'],
                      title))
